<?php
// Titulo
$_['header_title'] = 'Historial de pedidos';

// Texto
$_['text_account'] = 'Cuenta';
$_['text_order'] = 'Información del pedido';
$_['text_order_detail'] = 'Detalles del pedido';
$_['text_invoice_no'] = 'Número de factura:';
$_['text_order_id'] = 'ID de pedido:';
$_['text_date_added'] = 'Fecha de adición:';
$_['text_shipping_address'] = 'Dirección de envío';
$_['text_shipping_method'] = 'Método de envío:';
$_['text_payment_address'] = 'Dirección de pago';
$_['text_payment_method'] = 'Método de pago:';
$_['text_comment'] = 'Comentarios de pedido';
$_['text_history'] = 'Historial de pedidos';
$_['text_success'] = 'Éxito: ha agregado <a href="%s"> %s </a> a su <a href="%s"> carrito de compras </a>!';
$_['text_no_results'] = '¡No ha realizado ningún pedido anterior!';
$_['text_error'] = '¡No se pudo encontrar el pedido que solicitó!';

// columna
$_['column_order_id'] = 'ID de pedido';
$_['column_customer'] = 'Cliente';
$_['column_product'] = 'No. de productos';
$_['column_name'] = 'Nombre del producto';
$_['column_model'] = 'Modelo';
$_['column_quantity'] = 'Cantidad';
$_['column_price'] = 'Precio';
$_['column_total'] = 'Total';
$_['column_action'] = 'Acción';
$_['column_date_added'] = 'Fecha de adición';
$_['column_status'] = 'Estado';
$_['column_comment'] = 'Comentario';

// Error
$_['error_reorder'] = ' %s no está disponible actualmente para ser reordenado.';
